...
func (ctx *context) processKeyEvent(e gkey.Event, textbuffer *bytes.Buffer) {
	if e.Direction == gkey.DirRelease {
		return
	}
	...
	evinText := func() {
		if e.Modifiers == 0 || e.Modifiers == gkey.ModShift {
	...
	switch {
	case e.Code == gkey.CodeUnknown:
		if e.Rune > 0 {
			evinText()
		}
	case (e.Code >= gkey.CodeA && e.Code <= gkey.Code0) || e.Code == gkey.CodeSpacebar || e.Code == gkey.CodeHyphenMinus || e.Code == gkey.CodeEqualSign || e.Code == gkey.CodeLeftSquareBracket || e.Code == gkey.CodeRightSquareBracket || e.Code == gkey.CodeBackslash || e.Code == gkey.CodeSemicolon || e.Code == gkey.CodeApostrophe || e.Code == gkey.CodeGraveAccent || e.Code == gkey.CodeComma || e.Code == gkey.CodeFullStop || e.Code == gkey.CodeSlash || (e.Code >= gkey.CodeKeypadSlash && e.Code <= gkey.CodeKeypadPlusSign) || (e.Code >= gkey.CodeKeypad1 && e.Code <= gkey.CodeKeypadEqualSign):
		evinText()
	case e.Code == gkey.CodeTab:
		e.Rune = '\t'
		evinText()
	case e.Code == gkey.CodeReturnEnter || e.Code == gkey.CodeKeypadEnter:
		e.Rune = '\n'
		evinText()
	default:
		evinNotext()
...
func (ctx *context) Reset() {
	...
	if prevNumWindows == 2 && len(ctx.Windows) == 1 && ctx.Input.Mouse.valid {
		ctx.DockedWindows.Walk(func(w *Window) *Window {
			...
			for _, b := range []pointer.Buttons{pointer.ButtonLeft, pointer.ButtonRight, pointer.ButtonMiddle} {
	...
	in := &ctx.Input
	in.Mouse.Buttons[pointer.ButtonLeft].Clicked = false
	in.Mouse.Buttons[pointer.ButtonMiddle].Clicked = false
	in.Mouse.Buttons[pointer.ButtonRight].Clicked = false
