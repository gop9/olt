...
func (rtxt *RichText) handleClick(w *framework.Window, r rect.Rect, in *framework.Input, styleSel styleSel, line *line, chunkIdx int, hovering *bool, linkClick *int32) {
	...
	if rtxt.down {
		...
		if in.Mouse.Down(pointer.ButtonLeft) && in.Mouse.HoveringRect(r) {
			rtxt.focused = true
			q := line.coordToIndex(in.Mouse.Pos, chunkIdx, rtxt.adv)
...
func (rtxt *RichText) handleKeyboard(in *framework.Input) (arrowKey, pageKey int) {
	...
	for _, k := range in.Keyboard.Keys {
		switch {
		case k.Modifiers == gkey.ModControl && k.Code == gkey.CodeC:
			if rtxt.flags&Clipboard != 0 {
				clipboard.Set(rtxt.Get(rtxt.Sel))
			}
		case k.Code == gkey.CodeUpArrow:
			return -1, 0
		case k.Code == gkey.CodeDownArrow:
			return +1, 0
		case k.Code == gkey.CodePageDown:
			return 0, +1
		case k.Code == gkey.CodePageUp:
			return 0, -1
		}
	}
