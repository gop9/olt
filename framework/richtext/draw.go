...
func (rtxt *RichText) drawWidget(w *framework.Window) *Ctor {
	rtxt.first = false

	var flags framework.WindowFlags = 0
	if rtxt.flags&AutoWrap != 0 {
		flags = framework.WindowNoHScrollbar
	}
...
func (rtxt *RichText) drawRows(w *framework.Window, viewporth int) *Ctor {
	...
	if rtxt.flags&Keyboard != 0 && (w.Input().Mouse.Buttons[pointer.ButtonLeft].Down || w.Input().Mouse.Buttons[pointer.ButtonRight].Down) {
		rtxt.focused = false
	}
	...
	if rtxt.down && !in.Mouse.Down(pointer.ButtonLeft) {
		rtxt.down = false
	}
	...
	if pageKey != 0 && viewporth > 0 {
		scrollbary += (pageKey * viewporth) - framework.FontHeight(rtxt.face)
		if scrollbary < 0 {
			scrollbary = 0
		}
	} else if arrowKey != 0 {
		scrollbary += arrowKey * framework.FontHeight(rtxt.face)
		if scrollbary < 0 {
			scrollbary = 0
		}
	}
...
func (rtxt *RichText) drawTick(w *framework.Window, out *command.Buffer, p image.Point, lineh int, color color.RGBA, lineidx int) {
	linethick := int(w.Master().Style().Scaling)
	out.StrokeLine(image.Point{p.X, p.Y}, image.Point{p.X, p.Y + lineh}, linethick, color)
}

func (rtxt *RichText) drawSelectedChunk(w *framework.Window, out *command.Buffer, p *image.Point, chunk chunk, styleSel styleSel, width, lineh, lineasc int) {
	styleSel.BgColor = rtxt.selColor
	styleSel.Color = styleSel.SelFgColor
	drawChunk(w, out, p, chunk, styleSel, width, lineh, lineasc)
}

func drawChunk(w *framework.Window, out *command.Buffer, p *image.Point, chunk chunk, styleSel styleSel, width, lineh, lineasc int) {
	if chunk.isspacing() {
...
func (rtxt *RichText) reflow() {
	...
	flushLine := func(runedelta int) {
		...
		if len(ln.chunks) == 0 {
			ln.h = framework.FontHeight(siter.styleSel.Face)
	...
	for i, chunk := range rtxt.chunks {
		...
		flushChunk := func(end int32, styleSel styleSel) {
			if start != end {
				...
				h = append(h, framework.FontHeight(styleSel.Face))
