...
func panelBegin(ctx *context, win *Window, title string) {
	...
	// window dragging
	if win.moving {
		if in == nil || !in.Mouse.Down(pointer.ButtonLeft) {
	...
	*layout = panel{}
	...
	if (win.flags&WindowMovable != 0) && win.toplevel() {
		...
		if in.Mouse.IsClickDownInRect(pointer.ButtonLeft, move, true) && !in.Mouse.IsClickDownInRect(pointer.ButtonLeft, closeButton, true) {
			win.moving = true
		}
	}
...
func (win *Window) specialPanelBegin() {
	...
	if win.flags&windowNonblock != 0 && !win.first {
		...
		in_panel := ctx.Input.Mouse.IsClickInRect(pointer.ButtonLeft, win.ctx.Windows[0].layout.Bounds)
		prevbody := win.Bounds
		prevbody.H = win.layout.Height
		in_body := ctx.Input.Mouse.IsClickInRect(pointer.ButtonLeft, prevbody)
		in_header := ctx.Input.Mouse.IsClickInRect(pointer.ButtonLeft, win.header)
...
func panelEnd(ctx *context, window *Window) {
	...
	if layout.Flags&WindowScalable != 0 {
		...
		if window.toplevel() {
			...
			} else if in != nil && in.Mouse.IsClickDownInRect(pointer.ButtonLeft, dsab.ScalerRect, true) {
				window.scaling = true
			}
		}
	}
...
func buttonBehaviorDo(state *nstyle.WidgetStates, r rect.Rect, i *Input, repeat bool) (ret bool) {
	...
	if *state == nstyle.WidgetStateActive {
		if exitstate == nstyle.WidgetStateHovered {
			if repeat {
				ret = i.Mouse.Down(pointer.ButtonLeft)
			} else {
				ret = i.Mouse.Released(pointer.ButtonLeft)
			}
		}
		if !i.Mouse.Down(pointer.ButtonLeft) {
			*state = exitstate
		}
	}
...
func scrollbarBehavior(state *nstyle.WidgetStates, in *Input, scroll, cursor, empty0, empty1 rect.Rect, scroll_offset float64, target float64, scroll_step float64, o orientation) float64 {
	...
	if *state == nstyle.WidgetStateActive {
		if !in.Mouse.Down(pointer.ButtonLeft) {
			*state = exitstate
		} else {
			switch o {
			case vertical:
	...
	} else if in.Mouse.IsClickInRect(pointer.ButtonLeft, empty0) {
		switch o {
		case vertical:
	...
	} else if in.Mouse.IsClickInRect(pointer.ButtonLeft, empty1) {
		var max float64
		switch o {
		case vertical:
			scroll_offset += float64(scroll.H)
...
func toggleBehavior(in *Input, b rect.Rect, state *nstyle.WidgetStates, active bool) bool {
	...
	if *state == nstyle.WidgetStateHovered && in.Mouse.Clicked(pointer.ButtonLeft, b) {
...
func (ed *TextEditor) propertyBehavior(ws *nstyle.WidgetStates, in *Input, property rect.Rect, label rect.Rect, edit rect.Rect, empty rect.Rect) (drag bool, delta int) {
	if ed.propertyStatus == propertyDefault {
		...
		} else if in.Mouse.IsClickDownInRect(pointer.ButtonLeft, label, true) {
			ed.propertyStatus = propertyDrag
		} else if in.Mouse.IsClickDownInRect(pointer.ButtonLeft, empty, true) {
			ed.propertyStatus = propertyDrag
		}
	}

	if ed.propertyStatus == propertyDrag {
		if in.Mouse.Released(pointer.ButtonLeft) {
...
func (win *Window) doProperty(property rect.Rect, name string, text string, filter FilterFunc, in *Input) (ret doPropertyRet, delta int, ed *TextEditor) {
	...
	if active && in.Keyboard.Pressed(gkey.CodeReturnEnter) {
		active = !active
	}
...
func (win *Window) ContextualOpen(flags WindowFlags, size image.Point, trigger_bounds rect.Rect, updateFn UpdateFn) *Window {
	...
	if trigger_bounds.W > 0 && trigger_bounds.H > 0 {
		if !win.Input().Mouse.Clicked(pointer.ButtonRight, trigger_bounds) {
			return nil
		}
	}
