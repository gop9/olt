...
A window can be opened with the following three lines of code:
 ...
 wnd := framework.NewMasterWindow(0, "Title", updatefn)
...
The update function is responsible for drawing the contents of the window as well as handling user events, this is usually done by calling methods of framework.Window.
...
Widgets are laid out left to right and top to bottom, each row has a layout that can be configured calling the methods of framework.rowConstructor (an instance of which can be obtained by calling the `framework.Window.Row` or `framework.Window.RowScaled`). There are three main row layout modes:
