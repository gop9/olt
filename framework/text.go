...
func (state *TextEditor) key(e gkey.Event, font font.Face, row_height int, area_height int) {
...
retry:
	...
	case gkey.CodeZ:
		...
		if e.Modifiers&gkey.ModCtrl != 0 {
			if e.Modifiers&gkey.ModShift != 0 {
				state.DoRedo()
				state.HasPreferredX = false
	...
	case gkey.CodeInsert:
		state.InsertMode = !state.InsertMode

	case gkey.CodeLeftArrow:
		if e.Modifiers&gkey.ModCtrl != 0 {
			if e.Modifiers&gkey.ModShift != 0 {
				if !state.hasSelection() {
	...
	case gkey.CodePageDown:
		if state.SingleLine {
			break
		}
		state.verticalCursorMove(e, font, row_height, +area_height/2)

	case gkey.CodePageUp:
		if state.SingleLine {
			break
		}
		state.verticalCursorMove(e, font, row_height, -area_height/2)

	case gkey.CodeDeleteForward:
		if readOnly {
			return
		}
		if state.hasSelection() {
	...
	case gkey.CodeDeleteBackspace:
		...
		case state.hasSelection():
			state.DeleteSelection()
		case e.Modifiers&gkey.ModCtrl != 0:
			state.SelectEnd = state.Cursor
...
func (state *TextEditor) verticalCursorMove(e gkey.Event, font font.Face, row_height int, offset int) {
	if e.Modifiers&gkey.ModShift != 0 {
		state.prepSelectionAtCursor()
...
func (ed *TextEditor) doEdit(bounds rect.Rect, style *nstyle.Edit, inp *Input, cut, copy, paste bool) (ret EditEvents) {
	...
	if ed.Flags&EditFocusFollowsMouse != 0 {
		...
		if inp != nil && inp.Mouse.Buttons[pointer.ButtonLeft].Clicked && inp.Mouse.Buttons[pointer.ButtonLeft].Down {
	...
	if ed.Active && inp != nil {
		...
		var autoscrollTop bool
		{
			...
			autoscrollTop = inp.Mouse.HoveringRect(a) && inp.Mouse.Buttons[pointer.ButtonLeft].Down
		}
		...
		var autoscrollBot bool
		{
			...
			a.H = FontHeight(font) / 2
			autoscrollBot = inp.Mouse.HoveringRect(a) && inp.Mouse.Buttons[pointer.ButtonLeft].Down
		}

		/* mouse click handler */
		if select_all {
			ed.SelectAll()
		} else if isHovered && inp.Mouse.Buttons[pointer.ButtonLeft].Down && inp.Mouse.Buttons[pointer.ButtonLeft].Clicked {
		...
		} else if isHovered && inp.Mouse.Buttons[pointer.ButtonLeft].Down && (indelta.X != 0.0 || indelta.Y != 0.0) {
		...
		clipboardModifier := gkey.ModCtrl
		if runtime.GOOS == "darwin" {
			clipboardModifier = gkey.ModMeta
		}

		for _, e := range inp.Keyboard.Keys {
			switch e.Code {
			case gkey.CodeReturnEnter:
				if ed.Flags&EditCtrlEnterNewline != 0 && e.Modifiers&gkey.ModShift != 0 {
					ed.Text([]rune{'\n'})
					cursor_follow = true
...
var clipboardModifier = func() gkey.Modifiers {
	if runtime.GOOS == "darwin" {
		return gkey.ModMeta
	}
	return gkey.ModCtrl
...
func (edit *TextEditor) popupFind() {
	...
	edit.win.Master().PopupOpen("Search...", WindowTitle|WindowNoScrollbar|WindowMovable|WindowBorder|WindowDynamic, rect.Rect{100, 100, 400, 500}, true, func(w *Window) {
		...
		for _, k := range kbd.Keys {
			switch {
			case k.Modifiers == clipboardModifier && k.Code == gkey.CodeG:
				edit.lookForward(true)
			case k.Modifiers == 0 && k.Code == gkey.CodeEscape:
				edit.SelectEnd = edit.SelectStart
				edit.Cursor = edit.SelectStart
				edit.Active = true
				w.Close()
