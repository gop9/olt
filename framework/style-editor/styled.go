...
func EditStyle(w framework.MasterWindow, flags framework.WindowFlags, saveFn func(string)) {
	w.PopupOpen("Style Editor", framework.WindowTitle|framework.WindowBorder|framework.WindowMovable|framework.WindowScalable|flags, rect.Rect{0, 0, 400, 600}, true, StyleEditor(w.Style(), saveFn))
}

func StyleEditor(style *nstyle.Style, saveFn func(string)) framework.UpdateFn {
	se := &styleEditor{style: style, saveFn: saveFn}
	return se.Update
}

func (se *styleEditor) Update(w *framework.Window) {
	if se.style == nil {
...
func (se *styleEditor) editStruct(w *framework.Window, val reflect.Value) bool {
	changed := false
	for i := 0; i < val.NumField(); i++ {
		...
		case *nstyle.Text, *nstyle.Button, *nstyle.Toggle, *nstyle.Selectable, *nstyle.Slider, *nstyle.Progress, *nstyle.Property, *nstyle.Edit, *nstyle.Scrollbar, *nstyle.Tab, *nstyle.Combo, *nstyle.Window, *nstyle.WindowHeader:
			if w.TreePush(framework.TreeTab, field.Name, false) {
...
func (se *styleEditor) editColor(w *framework.Window, name string, pc *color.RGBA) bool {
...
func newColorPicker(pc *color.RGBA) *colorPicker {
	...
	cp.ed.Flags = framework.EditField | framework.EditSigEnter | framework.EditSelectable | framework.EditClipboard
	cp.setBuffer()
...
func (cp *colorPicker) Edit(w *framework.Window) bool {
	a := cp.ed.Edit(w)
	if a&framework.EditCommitted != 0 {
...
func (cp *colorPicker) Picker(w *framework.Window) bool {
...
func colorProgress(w *framework.Window, x *int, slider *[256]color.RGBA, pimg **image.RGBA) bool {
	const maxval = 255
